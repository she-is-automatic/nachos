...
void Thread::Fork(VoidFunctionPtr func, _int arg)
{
    ...
    // 分配栈空间，使得SWITCH可以切换到该线程执行的程序
    StackAllocate(func, arg);

    // 关中断，将该线程加入就绪队列，将中断状态恢复到原状态
...
void Thread::Finish()
{
    ...
    // 设置当前线程为threadToBeDestroyed
    threadToBeDestroyed = currentThread;

    // sleep()将当前线程设置为阻塞状态，并调用Run(nextThread)
    // Run(next)切换到下一个线程执行，并在最后释放当前线程的资源
...
void Thread::Yield()
{
    ...
    // 在就绪队列中找到下一个能运行的线程，
    // 若有，则将当前线程设置为READY状态，放入就绪队列
    // 将下一个线程设置为当前线程，状态设置为RUNNING；
